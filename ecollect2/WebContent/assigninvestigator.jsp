...
<body ng-app="App" ng-controller="investigatorsCtrl">
	<!-- Fixed navbar -->
	...
	<div class="container">
			<div class="row">
				<div class="col-md-6">
					<form name="newrepoForm" ng-submit="newinvestigatorfunc()" class="form-vertical">
						<div class="form-group">
							<label>Service Provider:</label>
							<select class="form-control selectpicker show-tick"
								title="Choose a repossessor ..." multiple ng-model="newinvestigator.investigator" id="investigator" name="investigator"
								data-max-options="4" data-live-search="true">
							</select>
						</div>
						<div class="form-group">
							<label>Date of Instruction:</label>
							<input type="text" class="form-control" id="dateinst" name="dateinst" ng-model="newinvestigator.dateinst" readonly>
						</div>

						<div class="input-group">
							<label>Due date:</label>
							<input type="text" class="form-control form_date" id="duedate" name="duedate" ng-model="newinvestigator.duedate" required>
							<span class="input-group-addon" style="display:block">
									<span class="glyphicon glyphicon-calendar"></span></span>
						</div>
						<div class="checkbox">
						    <label><input type="checkbox" ng-model="newinvestigator.reviewdate"> Update review date</label>
						</div>
						<div class="form-group">
				...
				<div class="col-md-6">
					<div id="chart-container">Investigators-Charts will load here!</div>
				</div>
			</div>
	</div>
	<script type="text/javascript">
		$( document ).ready(function(){
				...
				$.ajax({
			        type:"GET",
			        url: urladdress + "/api/v2/get_a_provider/INVESTIGATORS",
			        dataType:"json",
			        success: function(data) {
			            $('#investigator').empty();
			            $.each(data,function (index, item) {
			                $('#investigator').append("<option value='"+ item.SPTITLE +"' data-subtext=' EndOfIndemnity:"+item.ENDOFINDEMNITY+"'>"+ item.SPTITLE +" ("+item.PENDING+")"+"</option>");
			                $('#investigator').selectpicker('refresh');
			            });
			        },complete: function() {}
			    });
