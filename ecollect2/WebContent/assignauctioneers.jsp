...
<body ng-app="App" ng-controller="auctioneerCrtl">
	<!-- Fixed navbar -->
	...
	<div class="container">
			<div class="row">
				<div class="col-md-6">
					<form name="newauctioneerForm" ng-submit="newauctioneerfunc()" class="form-vertical">
						<div class="form-group">
							<label>Service Provider:</label>
							<select class="form-control selectpicker show-tick"
								title="Choose a service provider ..." multiple ng-model="newauctioneer.auctioneer" id="auctioneer" name="auctioneer"
								data-max-options="4" data-live-search="true">
							</select>
						</div>
						<div class="form-group">
							<label>Date of Instruction:</label>
							<input type="text" class="form-control" id="dateinst" name="dateinst" ng-model="newauctioneer.dateinst" readonly>
						</div>

						<div class="input-group">
							<label>Follow up date:</label>
							<input type="text" class="form-control form_date" id="duedate" name="duedate" ng-model="newauctioneer.duedate" required>
							<span class="input-group-addon" style="display:block">
									<span class="glyphicon glyphicon-calendar"></span></span>
						</div>
						<div class="checkbox">
						    <label><input type="checkbox" ng-model="newauctioneer.reviewdate"> Update review date</label>
						</div>
						<div class="form-group">
				...
				<div class="col-md-6">
					<div id="chart-container">Auctioneers-Charts will load here!</div>
				</div>
			</div>
	</div>
		...
		$( document ).ready(function(){
				...
				$.ajax({
			        ...
			        success: function(data) {
			        	// console.log(data);
			            $('#auctioneer').empty();
			            //$('#auctioneer').append("<option value='' >Choose a service provider ...</option>");
			            $.each(data,function (index, item) {
			                $('#auctioneer').append("<option value='"+ item.SPTITLE +"' data-subtext=' EndOfIndemnity:"+item.ENDOFINDEMNITY+"'>"+ item.SPTITLE +" ("+item.PENDING+")"+"</option>");
			                $('#auctioneer').selectpicker('refresh');
			            });
			        },complete: function() {}
			    });
