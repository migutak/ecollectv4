...
<body ng-app="App" ng-controller="marketorCtrl">
	<!-- Fixed navbar -->
	...
	<div class="container">
			<div class="row">
				<div class="col-md-6">
					<form name="newmarketorForm" ng-submit="newmarketorfunc()" class="form-vertical">
						<div class="form-group">
							<label>Service Provider:</label>
							<select class="form-control selectpicker show-tick"
								title="Choose a marketor ..." multiple ng-model="newmarketor.marketor" id="marketor" name="marketor"
								data-max-options="4" data-live-search="true">
							</select>
						</div>
						<div class="form-group">
							<label>Open Market Value:</label>
							<input type="text" class="form-control" id="omv" name="omv" ng-model="newmarketor.omv">
						</div>
						<div class="form-group">
							<label>Forced Sale Value:</label>
							<input type="text" class="form-control" id="fsv" name="fsv" ng-model="newmarketor.fsv">
						</div>
						<div class="form-group">
							<label>Date of Instruction:</label>
							<input type="text" class="form-control" id="dateinst" name="dateinst" ng-model="newmarketor.dateinst" readonly>
						</div>

						<div class="input-group">
							<label>Due date:</label>
							<input type="text" class="form-control form_date" id="duedate" name="duedate" ng-model="newmarketor.duedate" required>
							<span class="input-group-addon" style="display:block">
							<span class="glyphicon glyphicon-calendar"></span></span>
						</div>
						<div class="checkbox">
						    <label><input type="checkbox" ng-model="newmarketor.reviewdate"> Update review date</label>
						</div>
						<div class="form-group">
				...
				<div class="col-md-6">
					<div id="chart-container">Marketors-Charts will load here!</div>
				</div>
			</div>
	</div>
	<script type="text/javascript">
		$( document ).ready(function(){
				...
				$.ajax({
			        type:"GET",
			        url: urladdress + "/api/v2/get_a_provider/AUCTIONEERS-MARKETORS-REPOSSESSORS",
			        dataType:"json",
			        success: function(data) {
			            $('#marketor').empty();
			            $.each(data,function (index, item) {
			                $('#marketor').append("<option value='"+ item.SPTITLE +"' data-subtext=' EndOfIndemnity:"+item.ENDOFINDEMNITY+"'>"+ item.SPTITLE +" ("+item.PENDING+")"+"</option>");
			                $('#marketor').selectpicker('refresh');
			            });
			        },complete: function() {}
			    });
