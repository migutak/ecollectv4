...
<body ng-app="App" ng-controller="debtcollectorsCtrl">
	<!-- Fixed navbar -->
	...
	<div class="page-header">
		<h1 class="container" ng-cloak>
			{{custname.CUSTNAME}} <small> CUSTNUMBER:{{custname.CUSTNUMBER}}</small>
		</h1>
	...
	<div class="container">
			<div class="row">
				<div class="col-md-6">
					<form name="newdebtcollectorForm" ng-submit="newdebtcollectorfunc()" class="form-vertical">
						<div class="form-group">
							<label>Service Provider:</label>
							<select class="form-control selectpicker show-tick"
								title="Choose a repossessor ..." multiple ng-model="newdebtcollector.debtcollector" id="debtcollector" name="debtcollector"
								data-max-options="4" data-live-search="true">
							</select>
						</div>
						<div class="form-group">
							<label>Date of Instruction:</label>
							<input type="text" class="form-control" id="dateinst" name="dateinst" ng-model="newdebtcollector.dateinst" readonly>
						</div>

						<div class="input-group">
							<label>Due date:</label>
							<input type="text" class="form-control form_date" id="duedate" name="duedate" ng-model="newdebtcollector.duedate" required>
							<span class="input-group-addon" style="display:block">
									<span class="glyphicon glyphicon-calendar"></span></span>
						</div>
						<div class="form-group">
							<label>Loan settlement account:</label>
							<input type="text" class="form-control" id="loansettlementaccount" name="loansettlementaccount" ng-model="newdebtcollector.loansettlementaccount">
						</div>
						<div class="form-group">
							<label>Region:</label>
							<select class="form-control" ng-model="newdebtcollector.region" id="region" name="region" required>
						...
						<div class="form-group">
							...
							<input type="text" class="form-control" id="address" name="address" ng-model="newdebtcollector.address">
						</div>
						<div class="form-group">
							<label>Account Number:</label>
							<input type="text" class="form-control" id="acc" name="acc" ng-model="newdebtcollector.acc">
						</div>
						<div class="form-group" >
							<label>Account Balance:</label>
							<input type="text" class="form-control" id="accbalance" name="accbalance" ng-model="newdebtcollector.accbalance">
						</div>
						<div class="checkbox">
						    <label><input type="checkbox" ng-model="newdebtcollector.reviewdate"> Update review date</label>
						</div>
						<div class="form-group">
				...
				<div class="col-md-6">
					<div id="chart-container">Debt Collectors-Charts will load here!</div>
				</div>
			</div>
	</div>
	<script type="text/javascript">
		$( document ).ready(function(){
				...
				$.ajax({
			        type:"GET",
			        url: urladdress + "/api/v2/get_a_provider/DEBT COLLECTORS",
			        dataType:"json",
			        success: function(data) {
			            $('#debtcollector').empty();
			            $.each(data,function (index, item) {
			                $('#debtcollector').append("<option value='"+ item.SPTITLE +"' data-subtext=' EndOfIndemnity:"+item.ENDOFINDEMNITY+"'>"+ item.SPTITLE +" ("+item.PENDING+")"+"</option>");
			                $('#debtcollector').selectpicker('refresh');
			            });
			        },complete: function() {}
			    });
