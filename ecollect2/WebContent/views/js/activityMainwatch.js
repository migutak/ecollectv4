      ...
      vm.submit = function(){ //function to call on form submit
        ...
        vm.upload = function (file) {
          ...
          Upload.upload({
                ...
                if(resp.data.error_code === 0){ //validate success --> changed resp.data.file_dest
...
if(rights === 'Admin' || rights === 'ADMIN' || rights === 'Reviewer'){
 ...
 StudentDataOp.getActivitywatch(accnumber).success(function (data) {//changed from getAccount2 on 14th oct
   ...
   var nationalid = data[0].NATIONID;
    ...
    $scope.dataIn.arrearsamount = $scope.accdata[0].OUSTBALANCE;
    ...
    //$scope.dataIn.rfd_other = $scope.accdata[0].EXCUSE_OTHER;
      ...
      /*if(owner == undefined){
        ...
        var arrears = $scope.accdata[0].OUSTBALANCE;
        var intarr = $scope.accdata[0].OUSTBALANCE;
        var penarr = $scope.accdata[0].OUSTBALANCE;
        var days = 0;
        var cust = $scope.accdata[0].CUSTNUMBER;
        var arocode = $scope.accdata[0].AROCODE;
        var branch = $scope.accdata[0].BRANCHCODE;
        var manager = 'MANAGER';
        ...
        $scope.dataIn.totalarrears = $scope.accdata[0].OUSTBALANCE;
        $scope.dataIn.daysinarr = 0;

        if($scope.accdata[0].CUSTNAME !== null || $scope.accdata[0].CUSTNAME !== undefined || $scope.accdata[0].CUSTNAME !== ''){
          var custname = $scope.accdata[0].CUSTNAME;
        } else{
          var custname = $scope.accdata[0].CUSTNAME;
        }
...
app.controller('mainCtrl', function($scope,$timeout,StudentDataOp, Notification){
	...
	var acc = accnumber;//document.getElementById("acc").value;
	var dept = document.getElementById("s_in_division").value;
  var rights = document.getElementById("s_in_division").value;
  var username = gusername;//document.getElementById("username").value;
