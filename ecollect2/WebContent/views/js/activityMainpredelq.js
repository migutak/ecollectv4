...
app.controller('filesCtrl', function($scope,$state,$window,Upload,fileUpload,StudentDataOp,ServerAddress){
        ...
        vm.upload = function (file) {
            Upload.upload({
                ...
                if(resp.data.error_code === 0){ //validate success
                    ...
                    $.ajax({
                      url: ServerAddress.address + '/api/addfile',
                      type: "post", 
                      data:{'custnumber':document.getElementById("cust").value,'doctype':$scope.up.doctype,'docdesc':$scope.up.uploadnote,'destpath':resp.data.data_resp.path,'colofficer':username,
                     'accnumber':document.getElementById("acc").value,'filetype':resp.data.data_resp.mimetype,'filename':resp.data.data_resp.originalname,'filesize':resp.data.data_resp.size},
                      success: function(response) {
                       getFiles(document.getElementById("cust").value);
                       //refresh list
                       $state.reload();
                      },
                      error: function(xhr) {
...
app.controller('filesEmpCtrl', function($scope,$timeout,$state,$stateParams,$window,Upload,fileUpload,StudentDataOp,ServerAddress){
  ...
  $scope.employercust = [];
                    ...
                    //StudentDataOp.getEmployercust(document.getElementById("empcode").value).success(function (data) {
                      $timeout(function() {
                        $.ajax({
                          ...
                          type: "post", 
                          data:{
                            'doctype':'Employer letter','docdesc':$scope.up.uploadnote,'empcode':$scope.up.empcode,'destpath':resp.data.data_resp.path,'colofficer':username,
        ...
        $scope.lookupemployer = function(empcode){
          StudentDataOp.getEmployer(empcode).success(function(data){
            console.log(empcode);
            if(data.length > 0){
              document.getElementById('empname').value = data[0].EMPNAME;
            }else{
              alert('Employer with empcode '+empcode+' not found');
            }
          })
        }   
});
...
app.controller('ActivityCtrl', function($scope,$filter,$http,$window,StudentDataOp,ServerAddress,$timeout,$mdDialog,$mdMedia){
	...
	//console.log('--> in ActivityCtrl ...'+username); --> June Tsusho 0705949296
    ...
    StudentDataOp.getAccount2(document.getElementById("acc").value).success(function (data) {
...
app.controller('mainCtrl', function($scope,$timeout,StudentDataOp, Notification){
	...
	StudentDataOp.getAccount2(acc).success(function (data) {
		//same id notification
  	  	  var nationalid = data[0].NATIONID;
		  StudentDataOp.sameIdnumbers(nationalid).success(function(data){
	      $scope.sameidnotification = data.length;
