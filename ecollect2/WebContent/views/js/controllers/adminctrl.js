...
app.controller('MasterCtrl', function($scope, $http, StudentDataOp,ServerAddress) {
...
app.controller('usersCtrl', function($scope, $http, StudentDataOp,ServerAddress) {
	$scope.dataIn = {};
	$scope.userInfo = {};
	...
	$scope.lookupfn = function() {
		StudentDataOp.UserInfo($scope.dataIn.username).success(function(data) {
			if (data.length > 0) {
				...
				$scope.dataIn.team = $scope.userInfo[0].TEAM;
	...
	$scope.updtUser = function() {
		...
		if ($scope.dataIn.expiry == "" || $scope.dataIn.expiry == undefined) {
			$scope.dataIn.expiry = '2029-01-31';
		}
...
// editbranchCtrl
app.controller('editbranchCtrl', function($scope, $state, $stateParams, $http,$window, ServerAddress, StudentDataOp) {
...
app.controller('newuserCtrl', function($scope, $http, $window, ServerAddress,
		StudentDataOp) {
	$scope.branch = [];
	$scope.departments = [];
	StudentDataOp.getAllBranch().success(function(data) {
...
app.controller('departmentCtrl', function($scope, $state, $http, ServerAddress,
		StudentDataOp) {
	$scope.departments = [];
	$scope.dataIn = [];
