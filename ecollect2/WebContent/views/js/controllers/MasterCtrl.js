...
.controller('viewallCtrl',function($scope,$http,$window,ServerAddress,StudentDataOp,$stateParams, $state, uiGridConstants){
  ...
  $scope.mySearchrrocode = function(keyEvent) {
	  	if (keyEvent.which === 13){
	  		$scope.searchbycustnumber = undefined;
	  		$scope.searchbyname = undefined;
	  		$scope.searchbyaccnumber = undefined;
	  		...
	  		if($scope.searchbyarocode !== undefined){
	  			console.log('searching by rrocode '+ $scope.searchbyarocode);
	  				  $state.go('searchbyrrocode',{'rrocode':$scope.searchbyrrocode});
	  				}else{
	  					console.log('no search key detected'); 
	  				}
...
.controller('searcharocodeCtrl', function($scope,$scope,$stateParams,$http,$window,ServerAddress,$stateParams, $state, uiGridConstants,StudentDataOp){
	...
	$scope.gridOptions.columnDefs = [
		...
		{name:'BRANCHNAME'},
		{name:'TELNUMBER'},
		{name:'OUSTBALANCE',type: 'number',cellFilter: 'number: 2',sort: {direction: uiGridConstants.ASC}},
		{name:'TOTALARREARS',type: 'number',cellFilter: 'number: 2'},
		{name:'NATIONID'},
		{name:'BUCKET'},
		{name:'COLOFFICER'}
...
.controller('searchrrocodeCtrl', function($scope,$scope,$stateParams,$http,$window,ServerAddress,$stateParams, $state, uiGridConstants,StudentDataOp){
	var rrocode = $stateParams.rrocode;
	var username = document.getElementById("s_in_username").value;
	...
	$scope.gridOptions = {  
		...
		enableGridMenu: true,
		enableSelectAll: true,
		exporterCsvFilename: 'searchbyrrocode.csv',
		fastWatch: true,
		onRegisterApi: function(gridApi){
			$scope.gridApi = gridApi;
		}
	...
	loadgridData(rrocode);

	function loadgridData(rrocode){
		$http({
			method: 'get',
			url: ServerAddress.address + '/api/v2/searchbyrrocode/'+rrocode,
			headers: {'Content-Type': 'application/json'}
		}).success(function (data) {
			$scope.gridOptions.data = data;
		}).error(function (err) {
...
.controller('searchidnumberCtrl', function($scope,$scope,$stateParams,$http,$window,ServerAddress,$stateParams, $state, uiGridConstants,StudentDataOp){
	...
	function loadgridData(idnumber){
		$http({
			method: 'get',
			url: ServerAddress.address + '/api/v2_status/searchbyidnumber/'+idnumber,
			headers: {'Content-Type': 'application/json'}
		}).success(function (data) {
			$scope.gridOptions.data = data;
		}).error(function (err) {
...
.controller('searchbynameCtrl', function($scope,$scope,$http,$window,$stateParams,ServerAddress,StudentDataOp,$stateParams, $state, uiGridConstants){
	...
	function loadgridData(name){
		$http({
			method: 'get',
			url: ServerAddress.address+'/api/v2_status/searchbyname/'+name,
			headers: {'Content-Type': 'application/json'}
		}).success(function (data) {
	    	  //console.log(data);
	    	  $scope.gridOptions.data = data;
	...
	StudentDataOp.getinvintructions2(id).success(function (data) {
      ...
      $scope.statusfunc = function(){
      	...
      	$.ajax({
      		...
      		type: "post", 
      		data:{'id' : $stateParams.id,'receiptdate' : document.getElementById('receiptdate').value,
      		'status' : $scope.newstatus.status,'serviceprovider' : statuspname,'cust' : data[0].CUSTNUMBER
      		,'owner' : username,'extensiondate':document.getElementById('extensiondate').value},
      		success: function(response) {
      			$timeout(function() {
      				$mdDialog.show(
      					$mdDialog.alert()
...
.controller('allocateCtrl', function($scope,$http,Upload,StudentDataOp,ServerAddress){
	...
	console.log('Upload username ===>'+username+'====>'+rights);
	...
	StudentDataOp.Users().success(function (data) {
	 ...
	 $scope.upload = function (file) {
	    ...
	    $scope.$watch("dataIn.Reviewdate", function (newval) {
	    	console.log(newval)
	    	if(newval==='ammend_reviewdate'){
	    		$scope.withreviewdate = true;
	    	}else{
	    		$scope.withreviewdate = false;
	    	}
	    });
...
/*
    ...
    $scope.openunactionedrpt = function(){
    	window.open(ServerAddress.urlreports + "/frameset?__report=unactioned.rptdesign&__title=Unactioned report CMD","_blank");
    }
  ...
  /*$scope.logout = function(){
  	...
  	$mdDialog.show(confirm).then(function() {
	    		...
	    		window.open(ServerAddress.address,"_self");
